...
PECAS = ['campo', 'campo_topo', 'cume', 'beira_baixo', 'beira_esq', 'beira_dir',
         'canto_esq', 'canto_dir']
...
def campo_topo(cores):
    """Água de trás: mesma telha, mais escura.

    É o que faz o telhado ler como duas águas em vez de um retângulo
    chapado — sem contraste entre os dois lados da cumeeira, o olho não
    enxerga caimento nenhum.
    """
    base, luz, sombra, barra = cores
    escurece = lambda c: tuple(max(0, int(v * 0.72)) for v in c)
    return campo(escurece(base), escurece(luz), escurece(sombra), escurece(barra))
